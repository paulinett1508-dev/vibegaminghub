// =====================================================================
// solar-system.js — Solar System Planet Picker
// =====================================================================
// Fiel ao original de @coding.stella (Solar System Planet Picker
// KeyFrame Animations). Imagens locais para contornar COEP.
// Encapsulado como IIFE com API abrir()/fechar() para o hub.
// =====================================================================
...
(function () {
    ...
    var PLANETS = {
        ...
        neptune: { title:'Neptune', description:"A planet for pirates; just narrowly made the cut when scientists decided nine planets was too many.",    tilt:31.03, gravity:8.9, hours:10, img:'assets/sprites/solar-system/8_neptune.jpg' }
    };
    ...
    function _animate(duration, fn) {
        ...
        function tick(now) {
            ...
            fn(progress);
            if (progress < 1) _animFrames.push(requestAnimationFrame(tick));
        }
    ...
    // SVG nav original: texto sobre curva, o SVG todo roda para centrar o planeta ativo
    function _buildSVGNav() {
    ...
    // ---- CSS fiel ao original (todos os seletores com prefixo #solar-system-root) ----
    ...
    function _buildCSS() {
        return [
            ...
            /* box-sizing global dentro do root */
            '#solar-system-root *,#solar-system-root *:before,#solar-system-root *:after{box-sizing:border-box;}',

            /* position:relative global (necessario para labels ::before dos detalhes) */
            /* — usamos !important para nao colidir com fixed/absolute explícitos */
            '#solar-system-root *{position:relative;}',

            /* Close btn — precisa de fixed com especificidade maior */
            '#solar-system-root .ss-close-btn{position:fixed;top:16px;right:16px;z-index:10001;background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.3);color:#fff;width:44px;height:44px;border-radius:50%;cursor:pointer;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(4px);transition:background .2s;}',
            '#solar-system-root .ss-close-btn:hover{background:rgba(255,255,255,.3);}',

            /* App */
            '#solar-app{height:100%;width:100%;display:grid;grid-template-columns:1fr;grid-template-rows:1fr 1fr;overflow:hidden;position:relative;}',

            /* Planet */
            '#solar-system-root .ss-planet{grid-column:1;grid-row:1/-1;overflow:hidden;height:100%;width:100%;display:grid;grid-template-columns:10% 40% 40% 10%;grid-template-rows:10% 1fr 1fr;grid-template-areas:"header header header header" "x title details y" "x planet photos photos";visibility:hidden;transition:visibility .01s linear var(--ss-duration);}',
            '#solar-system-root .ss-planet[data-active]{visibility:visible;opacity:1;transition-delay:0s;}',

            '#solar-system-root .ss-planet>.ss-planet-title{display:block;grid-area:title;position:relative;z-index:5;}',
            '#solar-system-root .ss-planet>.ss-planet-details{grid-area:details;display:flex;flex-direction:row;justify-content:space-between;position:relative;z-index:5;}',

            /* Figure: imagem sai para baixo como no original */
            '#solar-system-root .ss-planet>.ss-planet-figure{grid-column:1/-1;grid-row:planet;padding:0;margin:0 auto;position:relative;}',
            '#solar-system-root .ss-planet>.ss-planet-figure img{margin-bottom:-50%;max-width:100%;}',
            '#solar-system-root .ss-planet>.ss-planet-figure::after{content:"";position:fixed;bottom:0%;top:0%;width:100%;left:0;background:linear-gradient(to top,rgba(0,0,0,.7) 0%,transparent 30%);z-index:2;pointer-events:none;}',

            /* Details labels */
            '#solar-system-root .ss-detail{font-size:5vmin;width:3em;font-weight:400;display:flex;margin-left:.4em;flex-shrink:0;align-self:start;position:relative;}',
            '#solar-system-root .ss-detail:after{content:attr(data-postfix);}',
            '#solar-system-root .ss-detail:before{display:block;position:absolute;top:100%;margin-top:1rem;font-size:.75rem;text-transform:uppercase;opacity:.6;letter-spacing:1px;}',
            '#solar-system-root .ss-detail[data-detail=hours]:before{content:"Hours";}',
            '#solar-system-root .ss-detail[data-detail=gravity]:before{content:"Gravity";}',
            '#solar-system-root .ss-detail[data-detail=tilt]:before{content:"Tilt";}',

            /* Tipografia */
            '#solar-system-root .ss-planet-title h1{margin:0;}',
            '#solar-system-root .ss-planet .ss-planet-title .word{overflow:hidden;}',
            '#solar-system-root .ss-planet .ss-planet-title .char{transform:translateY(100%);display:inline-block;transition:transform var(--ss-duration) var(--ss-ease);transition-delay:calc(var(--char-index)*.1s);}',
            '#solar-system-root .ss-planet[data-active] .ss-planet-title .char{transform:translateY(0%);transition-delay:calc((var(--ss-duration)/2)+(var(--char-index)*.1s));}',

            /* Animacoes de entrada */
            '#solar-system-root .ss-planet .ss-planet-description{visibility:hidden;opacity:0;transform:translateY(1em);transition:transform var(--ss-duration) var(--ss-ease),opacity var(--ss-duration) linear,visibility .01s linear var(--ss-duration);}',
            '#solar-system-root .ss-planet[data-active] .ss-planet-description{opacity:1;transform:translateY(0);visibility:visible;transition-delay:var(--ss-duration),var(--ss-duration),0s;}',
            '#solar-system-root .ss-planet .ss-planet-details{visibility:hidden;}',
            '#solar-system-root .ss-planet[data-active] .ss-planet-details{opacity:1;transform:translateY(0);visibility:visible;transition-delay:0s;}',
            ...
            '@media (max-width:600px){#solar-system-root .ss-planet-nav svg{margin-bottom:-55%;}}',
            '#solar-system-root .ss-planet-nav tspan{cursor:pointer;fill:#FFF;pointer-events:auto;opacity:0;transition:opacity var(--ss-duration) linear;}',
            '#solar-system-root .ss-planet-nav tspan[x]{opacity:0.6;}',
            '#solar-system-root .ss-planet-nav tspan:hover{opacity:1;}',
            /* A rotacao do SVG usa --active definido via JS no #solar-app */
            '#solar-system-root .ss-planet-nav svg{transform-origin:center center;--range:160deg;transform:rotate(calc((var(--active,0)/var(--ss-nav-length,7))*(-1*var(--range))+(var(--range)/2)));transition:transform var(--ss-duration) var(--ss-ease);}',
        ].join('\n');
    }
    ...
    function _selectPlanet(planetKey) {
        ...
        var elActive = _root.querySelector('.ss-planet[data-active]');
    ...
    function _selectByIndex(i) {
        ...
        // --active no svg controla a rotacao
        var svg = _root.querySelector('.ss-planet-nav svg');
        if (svg) svg.style.setProperty('--active', i);
    ...
    // ---- Posicionar tspans ao longo do path ----

    function _initSVGNav() {
        var svg      = _root.querySelector('.ss-planet-nav svg');
        var navPath  = _root.querySelector('#ss-navPath');
        var tspans   = Array.from(_root.querySelectorAll('.ss-planet-nav tspan'));
        if (!svg || !navPath || !tspans.length) return;

        var len    = tspans.length - 1;
        svg.style.setProperty('--ss-nav-length', String(len));

        var totalLen = navPath.getTotalLength();
        var lastLen  = tspans[len].getComputedTextLength();
        var usable   = totalLen - lastLen;

        tspans.forEach(function (tspan, i) {
            var pct = i / len;
            tspan.setAttribute('x', String(pct * usable));
        });
    ...
    function _init() {
        ...
        _onKeyDown = function (e) {
            if (e.key === 'Escape')    { if (window.fecharJoguinhos) window.fecharJoguinhos(); else window.SolarSystemGame.fechar(); }
        ...
        _onTouchMove = function (e) {
            ...
            var active = _root.querySelector('.ss-planet[data-active]');
            if (!active) return;
            active.style.transform = 'translateX(' + (dx * 0.45) + 'px)';
            active.style.opacity   = String(1 - Math.min(Math.abs(dx) / 380, 0.45));
        };
    ...
    var SolarSystemGame = {
        abrir: function () {
            ...
            appDiv.appendChild(_buildSVGNav());
