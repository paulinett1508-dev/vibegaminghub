# Arco e Flecha — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Portar o jogo de arco e flecha do `bow-game.zip` para o padrão IIFE do projeto, com overlay fullscreen, áudio Web Audio API, suporte a touch, e registro no hub.

**Architecture:** IIFE em `jogos/arco.js` expondo `window.ArcoGame = { abrir, fechar }`. O SVG do jogo é injetado no overlay via innerHTML. Scripts GSAP (TweenMax + MorphSVGPlugin) carregados como tags `<script>` estáticas em `index.html`. Lógica original do jogo mantida intacta dentro de `initJogo()`.

**Tech Stack:** Vanilla JS (ES5), SVG, GSAP TweenMax 1.19.1, MorphSVGPlugin (CodePen CDN), Web Audio API.
...
|---|---|---|
| Criar | `jogos/arco.js` | IIFE completo: áudio, SVG, lógica do jogo, overlay, touch |
| Modificar | `index.html` | Adicionar `<script>` do TweenMax, MorphSVGPlugin e arco.js |
| Modificar | `jogos/joguinhos-modal.js` | Adicionar entrada no array `JOGOS` |

---

## Task 1: Criar `jogos/arco.js`

**Files:**
- Create: `jogos/arco.js`

- [ ] **Step 1: Criar o arquivo com o IIFE completo**

Criar `jogos/arco.js` com o seguinte conteúdo:
...
```bash
node --check jogos/arco.js
```

Esperado: sem output (sem erros de sintaxe).

- [ ] **Step 3: Commit**

```bash
git add jogos/arco.js
git commit -m "feat(arco): adicionar jogo arco e flecha com fisica GSAP e audio Web Audio API"
```

---

## Task 2: Adicionar scripts GSAP em `index.html` e registrar no hub

**Files:**
- Modify: `index.html:468-481`
- Modify: `jogos/joguinhos-modal.js` (array `JOGOS`)

- [ ] **Step 1: Adicionar scripts GSAP em `index.html`**

Localizar o bloco de scripts (linha ~468) e adicionar TweenMax + MorphSVGPlugin **antes** do bloco de scripts dos jogos, e `arco.js` após `aranha.js`:

Substituir:
```html
    <!-- Deteccao de performance (DEVE carregar antes dos jogos) -->
    <script src="jogos/perf-detect.js?v=1"></script>

    <!-- Scripts dos jogos -->
    <script src="jogos/penaltis.js?v=2"></script>
    <script src="jogos/escorpiao.js?v=5"></script>
    <script src="jogos/reptil.js?v=3"></script>
    <script src="jogos/pacman.js?v=6"></script>
    <script src="jogos/tamandua.js?v=1"></script>
    <script src="jogos/aranha.js?v=1"></script>
    <script src="jogos/sonic.js?v=1"></script>
...
```html
    ...
    <script src="jogos/tamandua.js?v=1"></script>
    <script src="jogos/aranha.js?v=1"></script>
    <script src="jogos/arco.js?v=1"></script>
    <script src="jogos/sonic.js?v=1"></script>
    <script src="jogos/megadrive.js?v=1"></script>
    <script src="jogos/atari.js?v=1"></script>
    <script src="jogos/joguinhos-modal.js?v=10"></script>
...
- [ ] **Step 2: Registrar no array `JOGOS` em `joguinhos-modal.js`**

Localizar a entrada `aranha` no array `JOGOS` (por volta da linha 82) e adicionar logo após:
...
```bash
git add index.html jogos/joguinhos-modal.js
git commit -m "feat(arco): registrar jogo arco no hub e carregar GSAP"
```
...
- [ ] **Step 1: Abrir o projeto no browser**

Abrir `index.html` diretamente no browser (ou via servidor local) e verificar no console (F12) que não há erros ao carregar a página.

Esperado: sem erros de console. `window.ArcoGame` deve estar definido.

- [ ] **Step 2: Testar o fluxo completo**

1. Clicar no card "Arco" no hub → overlay abre com fundo escuro e SVG do jogo visível
2. Clicar e arrastar o mouse para a esquerda → arco estica, som de tensão toca
3. Soltar o mouse → flecha é lançada, som de whoosh toca
4. Flecha acerta a zona externa do alvo → mensagem "HIT" aparece, som de ding toca
5. Lançar flecha no centro exato → mensagem "LOVE YOU" aparece, fanfarra de 3 notas toca
6. Lançar flecha sem acertar → mensagem "MISS YOU" aparece, som grave toca
7. Pressionar ESC → overlay fecha
8. Botão ✕ → overlay fecha
9. Reabrir o jogo → funciona normalmente (sem leaks)

- [ ] **Step 3: Verificar touch (se disponível)**

Em dispositivo móvel ou DevTools com simulação de touch:
1. Tocar e arrastar → arco estica
2. Levantar o dedo → flecha dispara
...
**Cobertura do spec:**
- ✅ IIFE encapsulado com `window.ArcoGame`
- ✅ Overlay fullscreen com botão ✕ e suporte ESC
- ✅ SVG original preservado integralmente
- ✅ GSAP carregado via script tags estáticas em index.html
- ✅ Touch via simulação de MouseEvent
- ✅ Áudio: tensão (aim), disparo (loose), acerto (hit), bullseye (loveyou), erro (miss)
- ✅ Cleanup completo no fechar(): listeners window + overlay + AudioContext
- ✅ Entrada no hub com icon `sports` e gradiente verde

**Placeholders:** nenhum — todo código está inline no plano.

**Consistência de tipos:** `SomArco`, `overlay`, `_onMouseDown`, `initJogo`, `abrir`, `fechar` — nomes consistentes ao longo do plano.
